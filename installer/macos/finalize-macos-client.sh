...
if [[ -d "$SCRIPT_DIR/Radio Vault.app" ]]; then
  DEFAULT_APP="$SCRIPT_DIR/Radio Vault.app"
  ENTITLEMENTS="$SCRIPT_DIR/RadioVault.entitlements"
else
  ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
  DEFAULT_APP="$ROOT/artifacts/macos/osx-arm64/Radio Vault.app"
  ENTITLEMENTS="$SCRIPT_DIR/RadioVault.entitlements"
fi
APP="${1:-$DEFAULT_APP}"
...
if [[ ! -d "$APP" ]]; then
  echo "Radio Vault.app was not found: $APP" >&2
  exit 1
fi

EXECUTABLE="$APP/Contents/MacOS/TheRadioVault"
chmod +x "$EXECUTABLE"

if [[ -z "$IDENTITY" ]]; then
  echo "Prepared an unsigned development bundle. Set DEVELOPER_ID_APPLICATION to sign it."
  exit 0
fi
...
echo "Final Mac Client archive: $FINAL_ZIP"
