...
public sealed class MobileMediaProxy : IDisposable
{
    ...
    public MobileMediaProxy(MobileServerClient server)
    {
        ...
        using (socket)
        {
            ...
            try
            {
                ...
                using var response = await _server.OpenResponseAsync(
                    serverPath, request.Range, cancellationToken).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken)
    {
        var headers = new StringBuilder()
            .Append("HTTP/1.1 ").Append((int)response.StatusCode).Append(' ')
            .Append(string.IsNullOrWhiteSpace(response.ReasonPhrase) ? "Response" : response.ReasonPhrase).Append("\r\n")
    ...
    {
        if (!response.Headers.TryGetValues(name, out var values) && !response.Content.Headers.TryGetValues(name, out values)) return;
