...
Status: implementation complete; release soak pending. Mobile playback ownership, handoff evidence, multipart timeline mapping, remote-playhead observation, committed source-stop acknowledgement, library metadata synchronization, offline mutation replay, download lifecycle, downloaded-progress reconciliation, Explore, Knowledge, pairing, artwork hydration and Library query/direct-lookup rules now live in focused, behaviour-tested components while `MobileClientSession` remains the public façade. The web-server split has focused player/playback/queue, authenticated federation/administration, client API and media route boundaries. A pure lifecycle resolver now protects the order-sensitive pairing, authorization, HTTP-to-HTTPS, static-shell and API boundary, while one archive-discovery projection supplies Dashboard, On This Day and unheard results. Bounded HTTP request reading and response framing remain dedicated components with explicit timeout, malformed-framing and size-limit behavior. Desktop transport state, remote playhead smoothing and typed entity navigation use platform-neutral policies. The focused Mobile runner owns 43 checks and the compiled Web runner owns 114 checks alongside the Data, Services, Library Truth and Transcription suites. Schema 48 establishes the transactional numbered-migration boundary without rewriting the historical schema-47 bootstrap.
...
- Keep `WebRequestLifecycleResolver` as the sole order-sensitive request boundary and `WebArchiveDiscoveryProjection` as the canonical Dashboard/archive-discovery projection. Future server extraction must remove duplicated policy or side effects, not merely move methods into another file.
...
- Continue moving coherent behavioral groups from the remaining broad smoke runner into subsystem test projects; each migrated check must remain in exactly one runner.
...
Status: implementation complete; device acceptance pending. Core owns one typed entity link and navigation policy for articles, shows, broadcasts, people, topics, images and timelines, including stable identity, an actionable target and a deterministic Radio Vault deep link. Broadcast Info and Explore prose emit these links; desktop, iOS and Web resolve them while retaining compatibility fallbacks for older documents. Library transcript search returns the first matching timed phrase and both desktop and iOS preserve that requested position through playback startup. iOS Knowledge now offers cached/live coverage heat maps, month/day drill-down, broadcast navigation and swipe triage. Artwork hydration is cache-first and coalesced by episode identity, and Explore controllers no longer rebuild stable articles and dashboards on unrelated playback/sync notifications. Dashboard, On This Day and unheard discovery use one server projection.
