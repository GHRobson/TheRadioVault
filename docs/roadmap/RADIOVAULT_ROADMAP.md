...
- Continue splitting `LocalWebServer`; playback/queue, federation/administration, client and media route groups plus static web assets are extracted, leaving general API dispatch and shared HTTP infrastructure as the next boundaries.
...
- Continue moving source inspection into `TheRadioVault.SourceChecks` and behavioral checks from the remaining 9,342-line smoke runner into subsystem test projects; `TheRadioVault.Web.Tests` is the established pattern and next receives HTTP fixture behaviour.
