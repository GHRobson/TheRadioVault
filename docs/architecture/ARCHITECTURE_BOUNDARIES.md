...
`MobileDownloadCoordinator` owns transfer lifecycle and presentation state for downloads, including Wi-Fi eligibility, the durable date/episode watermark for new-only automatic downloads, pause/resume/cancel, age/completion/storage retention, protected current media and projection of the durable download index. `MobileDownloadService` remains the filesystem implementation behind its narrow store boundary. The mobile session decides whether the wider paired/playback workflow permits an operation and delegates the download operation itself; the iOS view only presents policy choices.

`NativeDownloadService` is the desktop filesystem and atomic-transfer boundary. It applies completion, expiry and least-recently-used storage rules while serializing them against active transfers. `DownloadsViewModel` sequences desktop automatic-download discovery and presentation, while `INativeDownloadPreferencesStore` durably owns each desktop/server-scope watermark and retention choice. The authoritative server Library is never deleted by device cleanup, and policy code must always pass the currently playing episode as protected media.
