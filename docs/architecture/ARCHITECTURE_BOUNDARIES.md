...
`MobileClientSession` remains the mobile client façade and retains network and decoder side effects. `MobilePlaybackOwnershipCoordinator` owns the pure rules that interpret a shared playback session: active ownership, committed moves, source-stop acknowledgement and the two-sample safeguard for legacy foreign owners. New ownership rules and evidence thresholds belong in that coordinator with focused tests; they must not be duplicated in the iOS view layer or folded back into the session façade.
