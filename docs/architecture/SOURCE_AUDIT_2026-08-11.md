...
|---|---|---|---|
| P1 | Web server | `LocalWebServer.cs` began at 10,436 lines. Player handoff/playback/queue, federation/administration, client API and media routing now live in focused partials, while the web client, service worker and secure setup page are embedded resource files. Bounded request parsing and response framing now live in dedicated, compiled-testable HTTP components; the main coordinator is 1,816 lines. | Continue separating general API dispatch while preserving dispatcher order, routes, range handling and protocol models. Stream future large pack imports rather than adding further whole-package buffering. |
...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` has fallen from 9,874 to 7,294 lines. Ten intentional route, streaming, Knowledge-integration, presentation and packaging inspections live in the dependency-free `TheRadioVault.SourceChecks` runner. Eighty-four Web query/model, HTTP-infrastructure, live HTTP, server-lifecycle, embedded-asset and playback-transfer checks now live in `TheRadioVault.Web.Tests`, which references only the Web subsystem. Its server, source-tree and archive-provider fixtures are reusable and the provider implementation is split by responsibility. All are release-gated. | Extract the next coherent behavioral group into its owning subsystem; do not duplicate migrated checks or replace one monolith with another. |
