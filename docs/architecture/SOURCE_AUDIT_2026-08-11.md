...
| P1 | Mobile session | `MobileClientSession.cs` began at 3,118 lines and still combines pairing, cache, broader sync, downloads, Explore, Knowledge, offline mutations and UI projection. Playback ownership, multipart timeline state, remote observation and committed source-stop acknowledgement are now extracted, reducing the façade to 2,983 lines. | Continue with broader sync, download and Explore coordinators, keeping the session as a façade. |
