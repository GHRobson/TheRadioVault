...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` has fallen from 9,874 to 9,342 lines. Ten intentional route, streaming, Knowledge-integration, presentation and packaging inspections live in the dependency-free 485-line `TheRadioVault.SourceChecks` runner. Sixteen Web query/model, route-contract and pure transactional handoff checks now live in the focused 347-line `TheRadioVault.Web.Tests` runner, which references only the Web subsystem. All are release-gated. | Move the Web HTTP fixture behaviour next, then extract other coherent behavioral groups; do not duplicate migrated checks or merely redistribute one monolith into another. |
