...
| P1 | Mobile session | `MobileClientSession.cs` began at 3,118 lines and still combines pairing, download reconciliation, Explore, Knowledge, offline mutations and UI projection. Playback ownership, multipart timeline state, remote observation, committed source-stop acknowledgement and serialized library change-feed synchronization are now extracted, reducing the façade to 2,916 lines. | Continue with offline-mutation, download and Explore coordinators, keeping the session as a façade. |
