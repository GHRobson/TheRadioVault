...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` has fallen from 9,874 to 7,294 lines. Eleven intentional route, streaming, Knowledge-integration, presentation and packaging inspections live in the dependency-free `TheRadioVault.SourceChecks` runner. Ninety-three Web query/model, route-resolution, HTTP-infrastructure, live HTTP, server-lifecycle, embedded-asset and playback-transfer checks now live in `TheRadioVault.Web.Tests`, which references only the Web subsystem. Its server, source-tree and archive-provider fixtures are reusable, use atomic OS-assigned ports, and split the provider implementation by responsibility. All are release-gated. | Extract the next coherent behavioral group into its owning subsystem; do not duplicate migrated checks or replace one monolith with another. |
