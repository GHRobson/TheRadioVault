...
| P1 | Mobile session | `MobileClientSession.cs` began at 3,118 lines and still combines pairing, download reconciliation, Explore, Knowledge and UI projection. Playback ownership, multipart timeline state, remote observation, committed source-stop acknowledgement, serialized library change-feed synchronization and durable offline-mutation replay are now extracted, reducing the façade to 2,854 lines. | Continue with download and Explore coordinators, keeping the session as a façade. |
