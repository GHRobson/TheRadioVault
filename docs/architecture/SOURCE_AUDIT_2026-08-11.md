...
| P1 | Mobile session | `MobileClientSession.cs` began at 3,118 lines. Playback ownership, multipart timeline state, remote observation, committed source-stop acknowledgement, serialized library change-feed synchronization, durable offline-mutation replay, download lifecycle, downloaded-progress authority reconciliation, Explore queries/image hydration and Knowledge queries/fallback coverage are now extracted, reducing the façade to 2,332 lines. | Continue with pairing and Library/UI projection boundaries while keeping the session as the public façade. |
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` is 9,771 lines. Some checks exercise behaviour; others only search source text for markers. | Move behavioural tests into normal test projects by subsystem. Retain a small packaging/source-boundary smoke runner where static inspection is intentional. |
