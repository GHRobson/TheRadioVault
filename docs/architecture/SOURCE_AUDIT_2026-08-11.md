...
|---|---|---|---|
| P1 | Web server | `LocalWebServer.cs` began at 10,436 lines and still combines authentication, HTML, CSS, JavaScript, media and many API families. Player handoff, playback command and queue routing/handlers are now isolated in `LocalWebServer.PlaybackQueue.cs`, reducing the main file to 10,150 lines. | Continue moving static web assets into resource files and splitting the remaining sync, knowledge and media route groups. Preserve dispatcher order, routes and protocol models. |
...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` is 9,784 lines. Some checks exercise behaviour; others only search source text for markers. | Move behavioural tests into normal test projects by subsystem. Retain a small packaging/source-boundary smoke runner where static inspection is intentional. |
