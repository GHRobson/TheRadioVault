...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` is 9,874 lines. Some checks exercise behaviour; others only search source text for markers. | Move behavioural tests into normal test projects by subsystem. Retain a small packaging/source-boundary smoke runner where static inspection is intentional. |
