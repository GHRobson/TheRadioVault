...
|---|---|---|---|
| P1 | Web server | `LocalWebServer.cs` is 10,436 lines and combines routing, authentication, HTML, CSS, JavaScript, media and API handling. | Move static web assets into resource files, then split route groups into library, playback, sync, knowledge and administration handlers. Preserve routes and protocol models. |
| P1 | Mobile session | `MobileClientSession.cs` began at 3,118 lines and still combines pairing, Explore, Knowledge and UI projection. Playback ownership, multipart timeline state, remote observation, committed source-stop acknowledgement, serialized library change-feed synchronization, durable offline-mutation replay, download lifecycle and downloaded-progress authority reconciliation are now extracted, reducing the façade to 2,632 lines. | Continue with Explore and Knowledge query coordinators, keeping the session as a façade. |
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` is 9,638 lines. Some checks exercise behaviour; others only search source text for markers. | Move behavioural tests into normal test projects by subsystem. Retain a small packaging/source-boundary smoke runner where static inspection is intentional. |
| P2 | Desktop playback | `PlaybackViewModel.cs` is 2,168 lines and coordinates state, UI commands, handoff and timing. | Introduce an explicit playback state machine and remote-progress interpolator, then keep Avalonia binding projection in the view model. |
