...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` has fallen from 9,874 to 9,626 lines. Ten intentional route, streaming, Knowledge-integration, presentation and packaging inspections now live in the dependency-free 485-line `TheRadioVault.SourceChecks` runner and execute through the release gates. Compiled resource loading remains behaviorally tested. | Continue moving source-only checks into SourceChecks and behavioral tests into normal subsystem projects; do not merely redistribute one monolith into another. |
