...
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` has fallen from 9,874 to 7,033 lines. Eleven intentional route, streaming, Knowledge-integration, presentation and packaging inspections live in the dependency-free `TheRadioVault.SourceChecks` runner. Ninety-three Web query/model, route-resolution, HTTP-infrastructure, live HTTP, server-lifecycle, embedded-asset and playback-transfer checks live in `TheRadioVault.Web.Tests`. Eight database seed, schema, upgrade, backup, order, idempotence and rollback checks now live in `TheRadioVault.Data.Tests`, using declarative object/column assertions instead of repeated command plumbing. Both subsystem suites are release-gated. | Extract the next coherent behavioral group into its owning subsystem; do not duplicate migrated checks or replace one monolith with another. |
