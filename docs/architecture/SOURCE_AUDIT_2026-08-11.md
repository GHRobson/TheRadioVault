...
|---|---|---|---|
| P1 | Web server | `LocalWebServer.cs` began at 10,436 lines. Player handoff/playback/queue, federation/administration, client API, media routing, trust/pairing policy and durable mutation acknowledgements now live in focused components, while the web client, service worker and secure setup page are embedded resource files. General API recognition, identifier capture and method policy live in a pure resolver and focused dispatcher. Bounded request parsing and response framing are dedicated, compiled-testable HTTP components; large Research and Wiki uploads are staged to temporary files and streamed into their import services under renewable inactivity deadlines. The main coordinator is now 1,429 lines. | Extract the next coherent request-lifecycle or handler family only when compiled behaviour tests can protect it. Preserve authentication order, routes, range handling and protocol models. Keep large imports stream-based and move large exports to streamed responses before their package limits grow. |
| P1 | Mobile session | `MobileClientSession.cs` began at 3,118 lines. Playback ownership, multipart timeline state, remote observation, committed source-stop acknowledgement, library synchronization, offline mutation replay, downloads, downloaded-progress authority, Explore, Knowledge, pairing and Library query/projection rules are now extracted, reducing the façade to 2,176 lines. | Keep the session as the public high-level orchestrator; extract another boundary only where focused behaviour tests justify it. |
| P1 | Shared test runner | `TheRadioVault.Tests/Program.cs` has fallen from 9,874 to 5,776 lines. Eleven intentional route, streaming, Knowledge-integration, presentation and packaging inspections live in the dependency-free `TheRadioVault.SourceChecks` runner. Compiled Web, Data and Transcription suites own their focused subsystem checks, and the new `TheRadioVault.LibraryTruth.Tests` runner owns 40 cross-platform parser, grouping, adoption and rehearsal behaviours. Every focused suite is release-gated and migrated checks remain in exactly one runner. | Extract the next coherent behavioral group into its owning subsystem; do not duplicate migrated checks or replace one monolith with another. |
