...
$zipPath = Join-Path $artifactRoot "RadioVault.Client-$version-$RuntimeIdentifier-unsigned.zip"
...
if ($SkipPublish) {
    ...
    foreach ($generatedPath in @(
        $bundleRoot,
        $zipPath,
        (Join-Path $artifactRoot "manifest.json"),
        (Join-Path $artifactRoot "RadioVault.entitlements")
    )) {
...
if (-not $SkipPublish) {
    & dotnet publish $project `
        ...
        -o $publishRoot
    if ($LASTEXITCODE -ne 0) { throw "The macOS client publish failed with exit code $LASTEXITCODE." }
}

$executable = Join-Path $publishRoot "TheRadioVault"
...
$logoPath = Join-Path $root "TheRadioVault.Desktop.Avalonia\Assets\RadioVault-Logo.png"
Copy-Item -LiteralPath $logoPath -Destination (Join-Path $resourcesRoot "RadioVault.png") -Force
Write-Icns $logoPath (Join-Path $resourcesRoot "RadioVault.icns")
Copy-Item -LiteralPath (Join-Path $root "installer\macos\RadioVault.entitlements") `
    -Destination (Join-Path $artifactRoot "RadioVault.entitlements") -Force
Copy-Item -LiteralPath (Join-Path $root "installer\macos\finalize-macos-client.sh") `
    -Destination (Join-Path $artifactRoot "finalize-macos-client.sh") -Force

$files = @(
    Get-ChildItem -LiteralPath $bundleRoot -Recurse -File
    Get-Item -LiteralPath (Join-Path $artifactRoot "RadioVault.entitlements")
    Get-Item -LiteralPath (Join-Path $artifactRoot "finalize-macos-client.sh")
...
Write-Host "Radio Vault Mac Client bundle created."
Write-Host "Bundle: $bundleRoot"
Write-Host "Unsigned transfer archive: $zipPath"
Write-Host "After extracting the ZIP on macOS, run: zsh finalize-macos-client.sh"
